...
class UserLogInController extends Controller
{
   ...
   function user_login(Request $request)
                                 ...
                                 if (($check_new_user->active == 0) || ($logdata->device == $device)) {
                                    ...
                                    if (count($bal) == 0) {
                                        ...
                                        foreach ($bal as $points) {
                                            $send_notification = $this->sendNotificationToAdmin($logdata->user_id);
                                            $send_notification =  $this->sendNotificationFCM($logdata->user_id);
                                            $message['message'] = 'Login Successfully';
                                            $message['status'] = "200";
    ...
    function sendNotificationToAdmin($user_id) { 
        $date = date('Y-m-d h:i:s');
        $NotificationArray = array('distributor_id' => $user_id,'points' => 0, 'status' => 4, 'created_at' => $date);
   ...
   function profile(Request $request)
    {
            ...
            $IMEI = $request->input('imei');
            $device = $request->input('device');
            $check_device = $this->CheckDeviceId ($username,$device);
           if($check_device){
                $login_data = DB::select('SELECT * FROM user WHERE user_id=?', [$username]);
                if (count($login_data) != 0) {
                    foreach ($login_data as $logdata) {
                        ...
                        if ($logdata->IsBlocked != 1) {
                            if (Hash::check($password, $logdata->password)) {
                                ...
                                foreach ($new_phone as $check_new_user) {
                                    ...
                                    if (count($bal) == 0) {
                                        $message['status']  = "200";
                                        $message['message'] = 'My Profile';
                                        $message['points']  =  "0";
                                        $message['device']  = $logdata->device;
                                        $message['user_id'] = $logdata->user_id;
                                        $response['user_data'] = $message;
                                    } else {
                                        foreach ($bal as $points) {
    ...
    // Function For Get Apk Veersion
    function get_apk_version()    
    { 
        $version = DB::table('setting')->first();
        if(!empty($version)){
            $message['status']  = "200";
            $message['message'] = 'Apk Version Code';
            $message['version_code'] = $version->apk_version;
            //$message['Apk Url'] = Url('/app/fungameasiaa.apk');
            $message['Apk Url'] ="https://www.fungameasiaa.com";

        } else {
            $message['status']  = "400";
            $message['message'] = 'Version not updated.';
        }
    ...
    // Function For User Notification
    function UserNotice(Request $request)    
    {
         $user_id = $request->input('user_id');
         $device  = $request->input('device');
        $check_device = $this->CheckDeviceId ($user_id,$device);
    ...
    function UpdateDeviceId(Request $request){
        try{
            ...
            $password = $request->input('password');
            $device = $request->input('device');
            $check_update_data = DB::update('UPDATE `user` SET `device`=? WHERE user_id = ?',[$device,$user_id]);
            if($check_update_data){
                $login_data = DB::select('SELECT * FROM user WHERE user_id=?', [$user_id]);
                // print_r($login_data);die
                if (count($login_data) != 0) {
                    foreach ($login_data as $logdata) {
                        ...
                        if ($logdata->IsBlocked != 1) {
                            if (Hash::check($password, $logdata->password)) {
                                $bal = DB::select('SELECT points FROM `user_points` WHERE user_id=?', [$user_id]);
                                $new_phone = DB::select('SELECT `device`, `last_logged_in` FROM `user` WHERE user_id=?', [$user_id]);
                                foreach ($new_phone as $check_new_user) {
                                    if ($check_new_user->device == NULL) {
                                        $last_logged_in = date('Y-m-d h:i:s');
                                        DB::update('UPDATE `user` SET `last_logged_in`=?, `device`=? WHERE user_id =?', [$last_logged_in, $device, $user_id]);
                                        if (count($bal) == 0) {
                                            $message['message'] = 'Login Successfully';
                                            $message['status'] = "200";
                                            $message['coins'] = "0";
                                        ...
                                        if ($check_new_user->device == $device) {
                                            $last_logged_in = date('Y-m-d h:i:s');
                                            DB::update('UPDATE `user` SET `last_logged_in`=? WHERE user_id =?', [$last_logged_in, $user_id]);
                                            if (count($bal) == 0) {
                                                $message['message'] = 'Login Successfully';
    ...
    { 
     ...
     public function test(){
        ...
        $response['body'] = 'User Notification';
        $response['title'] = 'FUN GAMES ASIA';
        $response['icon'] = "https://www.fungameasiaa.com/GameImage/logo.png";
        $response['badge'] = "https://nmsgames.com/img/fcm2.png";
        $response['click_action'] = "http://127.0.0.1:8000/";
        return response()->json($response);
